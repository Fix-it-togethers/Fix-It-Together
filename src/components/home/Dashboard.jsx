...
import { BiMessageEdit, BiSolidLeftArrowSquare, BiSolidRightArrowSquare } from 'react-icons/bi'
        ...
        <div className="w-full lg:ml-4 mt-10 lg:mt-14 text-[#061826]">
                        ...
                        records?.map(issue => {
                            return (
                                <Link to={`/Detail/${issue.id}`} key={issue.id}>
                                    <div className="w-[100%] mx-auto lg:mx-0 flex flex-col justify-start items-start lg:items-start transition hover:shadow-lg cursor-pointer ease-in-out p-2 shadow rounded border" key={issue.id}>
                                        <img className='w-full lg:w-72 lg:h-60' src={issue.image} alt="photo fix issue" />
                                        <div className=' space-y-1 mt-2 w-full'>
                                            <h1 className='text-xl font-medium'>{issue.title.slice(0, 30) + [issue.title.length >= 30 ? "..." : " "]}</h1>
                                            <p className='text-base w-11/12'>{issue.description.slice(0, 60) + [issue.description.length > 60 ? "  Read more..." : " "]} </p>
                                            <div className='flex flex-col justify-start items-start space-y-1 gap-1'>
                                                <p className='flex flex-row justify-start lg:items-center items-start'><CiFlag1 className='mt-1' size={20} /> <span className='ml-2'>{issue.location}</span></p>
            ...
            <nav className='mt-20'>
                <ul className='flex flex-row justify-start items-center gap-4'>
                    <li>
                        <button onClick={prePage} className='bg-[#061826] px-3 py-2 text-[#fff] rounded  shadow-lg'><BiSolidLeftArrowSquare size={20}/></button>
                    </li>
                    {
                        numbers.map((page, index) => {
                            return (
                                <li key={index} className={`${currentPage === page ? 'bg-[#061826] rounded px-4 py-1  text-[#fff] text-lg ' : ''}`}>
                    ...
                    <li>
                        <button onClick={nextPage} className='bg-[#061826] px-3 py-2  text-[#fff] rounded shadow-lg'><BiSolidRightArrowSquare size={20}/></button>
                    </li>
                </ul>
            </nav>
        </div>
